...
final class JsonParser {
    ...
    func parse(json: JSON, modelName: String) {
        let model = modelDataSource.modelGenerator(modelName: mappedClassNames(key: modelName))
...
private extension JsonParser {
    ...
    func parse(key: String, value: Any) -> Type? {
        if let array = value as? Array<Any> {
            return .array(mappedClassNames(key: parse(array: array, key: key)))
            ...
            return .custom(mappedClassNames(key: key))
        }        
        return nil
    }
    ...
    private func mappedClassNames(key: String) -> String {
        return config.mappedModelNames[key] ?? key
    }
