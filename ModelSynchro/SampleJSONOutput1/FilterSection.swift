...
struct FilterSection: Codable {
	...
	init(from decoder: Decoder) throws {
        ...
        do {
            ...
            throw ModelSynchroDecoderError.keyNotFound(description: "error: filters key is not found!")
        }

        do {
            section = try container.decode(String.self, forKey: .section)
        } catch {
            throw ModelSynchroDecoderError.keyNotFound(description: "error: section key is not found!")
        }
	}
}
